...
#define ISSE 1
//#undef ISSE // disable ISSE for debug
...
// added in v.3.2
extern "C"
void find_mot_line_isse(const BYTE * p, const BYTE * c, BYTE * motion, int width, int mthres )
{
	...
	__asm
	{
		...
		movd	mm5, [mthres] ; mm5 = 0  mthres (dwords)
							; mm5 = 0 0 0 mthres (words)
		pshufw	mm4, mm5, 0 ; mm4 = mthres mthres mthres mthres (words)
							; mm4 = 0 mthres 0 mthres 0 mthres 0 mthres (bytes)
		movq	mm5, mm4	; copy: mm5 = mthres mthres mthres mthres (words)
							; mm5 = 0 mthres 0 mthres 0 mthres 0 mthres (bytes)
		psllw	mm4, 8		; mm4 = mthres 0 mthres 0 mthres 0 mthres 0 (bytes)
		por		mm5, mm4 ; mm5 = mthres mthres mthres mthres mthres mthres mthres mthres (bytes)

		mov		eax, 0x03030303 ; eax = 03 03 03 03 (bytes)
		movd	mm3, eax	; mm3 = 0 0 0 0 03 03 03 03 (bytes)
		punpckldq mm3,mm3	; mm3 = 03 03 03 03   03 03 03 03

		xor		eax, eax
align 16
Lab1:
	movq	mm1, [esi+eax] ; mm1=p[x]
	movq	mm2, mm1 ; mm2=p
	movq	mm0, [ebx+eax] ; mm0=c[x]
	psubusb	mm2, mm0 ; mm2=p-c     resut=saturation signed bytes(-128 to 127)
	psubusb	mm0, mm1 ; mm0=c-p
	por		mm0, mm2 ; mm0= absolute unsigned dist= c-p OR p-c
	pcmpgtb	mm0, mm5 ; dist > thres ?
					; mm0 = all FF or all 0 (bytes)
	pand	mm0, mm3 ; mm0 = all 3 or all 0
	movq	[edi+eax], mm0 ; motion
	add		eax, 8
	cmp		eax, ecx
	jl		Lab1
...
//  BYTE mratio = parms.mratio; // was in v.2.1 and removed in 3.0
  ...
  {
#ifdef ISSE
	  find_mot_line_isse(p, c, motion, width, mthres);
#else
...
// added in v.3.2
extern "C"
void motion_summation_isse(unsigned char * moving, signed char * row, int w, int h, int y1, int y2)
{
//
//		if (y1 >= 0)
//		{
//		  unsigned char * r2 = moving + w * y1;
//		  for ( unsigned int x = 0; x != w; ++x)
//			r[x] -= r2[x];
//		}
//		if (y2 < h)
//		{
//		  unsigned char * r2 = moving + w * y2;
//		  for ( unsigned int x = 0; x != w; ++x)
//			r[x] += r2[x];
//		}
//
	__asm
	{
		...
		imul	eax, ecx // eax = w* y1
		add		esi, eax
		xor edx, edx
Lcycley1:
		movq	mm1,[edi + edx]     // all eight bytes from dest memory into mm1
		psubsb	mm1, [esi + edx]		// sub every bytes of 8
		movq	[edi+edx], mm1  // all eight bytes to  dest memory from mm1
		add		edx, 8
		sub		ecx, 8
		jns		Lcycley1
...
//		motion_summation(moving, r,  w,  h,  y1,  y2);
		motion_summation_isse(moving, r,  w,  h,  y1,  y2);
#else
		motion_summation(moving, r,  w,  h,  y1,  y2);
#endif
		if (y >= 0)
...
//		motion_summation(fmoving, r,  w,  h,  y1,  y2);
		motion_summation_isse(fmoving, r,  w,  h,  y1,  y2);
#else
		motion_summation(fmoving, r,  w,  h,  y1,  y2);
#endif
    if (y >= 0)
...
#ifdef ISSE
		motion_summation_isse(fmoving, r,  w,  h,  y1,  y2); // v.3.2
#else
...
// added in v.3.2
extern "C"
void median_line_isse(const BYTE * p,const BYTE * n,const BYTE * c, BYTE * motion, BYTE * o, int wmod8)
{
	__asm
	{
		push	ebp ; // added in v.3.3.3 to fix bug
		mov		esi,[p]
		mov		ebx,[n]
		mov		edx,[c]
		mov		edi,[o]
		...
		xor		eax, eax
Lcycle:
		movq	mm0,[esi + eax]     ;// all eight bytes from p  into mm0
		movq	mm1,[ebx + eax]     ;// all eight bytes from n  into mm1
		movq	mm2, mm0
		pminub	mm0, mm1 ; mm0=min(p,n)
		pmaxub	mm2, mm1 ; mm2=max(p,n)

		movq	mm1, [edx+eax] ;// c
		pmaxub	mm0, mm1 ; mm0 =max(min(p,n), c)
		pminub	mm2, mm0 ; mm2 =median=min(max(p,n), max(min(p,n),c))


		movq	mm3, [ebp+eax] // motion
		pand	mm1, mm3 ;// c & motion
		pandn	mm3, mm2 ;// notmotion & median

		por		mm1, mm3 ; (c & motion) | (notmotion & median)

		movq	[edi+eax], mm1  ;// all eight bytes
		add		eax, 8
		cmp		eax, ecx
		jl		Lcycle
		pop	ebp ; // added in v.3.3.3 to fix bug
	}
}
...
{{
  ...
  for (int y = 0; y != parms.height; ++y) {
#ifdef ISSE
	  median_line_isse(p, n, c, c_motion, o, width); // v.3.2
#else
...
// added in v.3.2
extern "C"
void getminmax_isse(const unsigned char * p, const unsigned char * n,
				   unsigned char * minpn, unsigned char * maxpn, int wmod8 )
{
...
Lminmaxcycle:
		...
		pminub	mm0, mm1
		pmaxub	mm2, mm1
		movq	[edi+eax], mm0  // all eight bytes to  minpn  from mm0
		movq	[edx+eax], mm2  // all eight bytes to  maxpn  from mm2
		add		eax, 8
		cmp		eax, ecx
		jl		Lminmaxcycle
		jmp		Lend
...
Lmaxcycle:
		...
		movq	mm1,[ebx + eax]     // all eight bytes from n  into mm1
		pmaxub	mm0, mm1
		movq	[edx+eax], mm0  // all eight bytes to  minpn  from mm0
		add		eax, 8
		cmp		eax, ecx
		jl		Lmaxcycle
		jmp		Lend
...
// added in v.3.2
extern "C"
void find_sdi_p1_isse(const unsigned char * p, const unsigned char * n, const unsigned char * c,
				unsigned char * o,   int wmod8 )
{
...
// added in v.3.2
extern "C"
void find_sdi_m1_isse(const unsigned char * p, const unsigned char * n, const unsigned char * c,
				unsigned char * o,   int wmod8 )
{
...
Lcycle:
		...
		movq	[edi+eax], mm1  // all eight bytes to  minpn  from mm0
		add		eax, 8
		cmp		eax, ecx
		jl		Lcycle
//		emms
...
// added in v.3.2
extern "C"
void find_sdi_0_isse(const unsigned char * p, const unsigned char * n, const unsigned char * c,
				unsigned char * o,   int wmod8 )
{
...
#ifdef ISSE
					getminmax_isse(p, n, minpn, 0, wmod8);
#else
...
#ifdef ISSE
					getminmax_isse(p, n, 0, maxpn, wmod8);
#else
...
#ifdef ISSE
					getminmax_isse(p, n, minpn, maxpn, wmod8);
#else
...
//							mx = max(mx,n[x+1]);// have max from 2+4=6 pixels
		...
		{ // old SDI method  by 2 pixels
			...
			switch (parms.sign)
			{
				case 1://  sign=1 - only black (dark) spots and outliers
#ifdef ISSE
					find_sdi_p1_isse(p, n, c, o, wmod8);
#else
				...
				case -1://  sign=-1 - only white (light) spots and outliers
#ifdef ISSE
					find_sdi_m1_isse(p, n, c, o, wmod8);
#else
...
//
extern "C"
void tsmooth_line_isse(const BYTE * p, const BYTE * n, BYTE * o, BYTE * c_noise, BYTE * c_motion, int width, int tsmoothLimit)
{
...
/*	for (size_t x = 0; x != width; ++x)
	{
			...
			// Algo is similar to FaeryDust?
			 int m = (p[x] + o[x] + n[x])/3;        // mean value (integer precise)
			 int corr = m - o[x];
			 if (corr > (tsmoothLimit<<1)) corr = 0;  // disable too large
			 if (corr < -(tsmoothLimit<<1)) corr = 0; // disable too large
			 if (corr > tsmoothLimit) corr = (tsmoothLimit<<1) - corr; // correction limit
			 if (corr < -tsmoothLimit) corr = -(tsmoothLimit<<1) - corr; // correction limit
			if ( !c_noise[x] && !c_motion[x]) o[x] += corr;
...
*/
	_asm
	{
		...
		mov		edx, [c_noise]
		mov		ecx, [width]
		mov		eax, [tsmoothLimit]
		movd	mm6, eax ; mm6 = tsmoothLimit
		pshufw	mm6, mm6, 0 ; tsmoothLimit tsmoothLimit tsmoothLimit tsmoothLimit

;// how to divide by 3? - replace by mult and shift   m/3 = m*85/256
		// more correct average is (int)(m+3)*85/256 = (m*85 + 85+85+85)/256 - fixed in v3.5
		mov		eax, 85
		movd	mm5, eax ; mm5 = 85
		pshufw mm5, mm5, 0 ; 85 85 85 85

		mov		eax, [c_motion]

Align 16
Lcycle:
		sub		ecx, 4 ; process in reverse direction
		js		Lend

		pxor	mm0, mm0 ; mm0 = 0

		movd	mm1, [esi+ecx] ; p:   0 0 0 0 p3 p2 p1 p0
		punpcklbw mm1, mm0	; 0 p3 0 p2 0 p1 0 p0
		...
		movq	mm4, mm1 ; p
		paddusw	mm4, mm2 ; p+n
		paddusw mm4, mm3 ; p+n+o

		pmullw	mm4, mm5 ; mm4 = (p+n+o) *85 low bits (no high)
		paddusw mm4, mm5; // + 85    fixed in v3.5
		paddusw mm4, mm5; // + 85    fixed in v3.5
		paddusw mm4, mm5; // + 85    fixed in v3.5
		psrlw	mm4, 8	; mm4 = mm4/256 = m = (p+n+o)/3

		movq	mm1, mm4 ; mm1 = m
		psubsw	mm1, mm3 ; mm1 = m-o = corr
		movq	mm2, mm1 ; mm2 = corr

		movq	mm7, mm6 ; mm7 = tsmoothLimit
		psllw	mm7, 1 ; mm7 = tsmoothLimit*2
		pcmpgtw mm2, mm7 ; corr > tsmoothLimit*2 ?
		pandn	mm2, mm1 ; mm2 = corr or 0(if was > tsmoothLimit*2)

		pxor	mm0, mm0 ; mm0 = 0
		psubsw	mm0, mm7 ; mm0= 0-tsmoothLimit*2
		pcmpgtw	mm0, mm2 ; -tsmoothLimit*2 > (corr checked)
		pandn	mm0, mm2 ; corr or 0 (if was < -tsmoothLimit*2)
		movq	mm2, mm0 ; mm2 = corr checked

		movq	mm7, mm6 ; mm7 = tsmoothLimit
		psllw	mm7, 1   ; mm7 = tsmoothLimit*2
		psubsw	mm7, mm2 ; mm7 = tsmoothLimit*2 - corr (checked)

		pcmpgtw mm2, mm6 ; corr > tsmoothLimit ?
		pand	mm7, mm2 ; (tsmoothLimit*2 - corr)  and (corr > tsmoothLimit)
		pandn	mm2, mm0 ; not (corr > tsmoothLimit) and corr
		por		mm2, mm7 ; corr (clipped )

		movq	mm7, mm6 ; mm7= tsmoothLimit
		psllw	mm7, 1 ; mm7 = tsmoothLimit*2
		pxor	mm0, mm0 ; mm0 = 0
		psubsw	mm0, mm7 ; mm0= 0-tsmoothLimit*2
		movq	mm7, mm0 ; mm7 = -tsmoothLimit*2
		psubsw	mm7, mm2 ; mm7 = -tsmoothLimit*2 - corr
		pxor	mm0, mm0 ; mm0 = 0
		psubsw	mm0, mm6 ; mm0 = -tsmoothLimit

		pcmpgtw	mm0, mm2 ; -tsmoothLimit > (corr) ?

		pand	mm7, mm0 ; (-tsmoothLimit*2 - corr) and (-tsmoothLimit > corr )
		pandn	mm0, mm2 ; not ( -tsmoothLimit > corr) and (corr)
		por		mm0, mm7 ; mm0= final clipped correction

		paddsw	mm0, mm3 ; mm0 = o + correction

;//  no noise and  moving condition must be here
		movd	mm1, [edx+ecx] ; noise
		pxor	mm4, mm4 ; mm4 = 0
		punpcklbw mm1, mm1	; noise3 noise3 noise2 noise2 noise1 noise1 noise0 noise0
		pcmpeqw	mm1, mm4 ; mm1 =(not noise3) (not noise2) (not noise1) (not noise0)   (words)
		movd	mm7, [eax+ecx] ; motion
		punpcklbw mm7, mm7	; motion3 motion3 motion2 motion2 motion1 motion1 motion0 motion0
		pcmpeqw	mm7, mm4 ; //mm7 = (not motion3)  (not motion2) (not motion1) (not motion0) (words)
		pand	mm1, mm7 ; //mm1 = static = not noise and not motion (words)

		pand	mm0, mm1 ; //smoothed & static
		pandn	mm1, mm3 ; //notstatic & o (original)
		por		mm0, mm1 ; //(smoothed & static | notstatic & o (original))

		packuswb mm0,mm0 ; pack to bytes: tmp3 tmp2 tmp1 tmp0 tmp3 tmp2 tmp1 tmp0
		movd [edi+ecx], mm0 ; write result to o
...
//		tsmooth_line(p, n, o, c_noise, c_motion,width, tsmooth); // 3.2
		tsmooth_line_isse(p, n, o, c_noise, c_motion, width, tsmooth); // 3.2
#else
		tsmooth_line(p, n, o, c_noise, c_motion,width, tsmooth); // 3.2
#endif
	}
...
// added in v.3.3
extern "C"
void motion_count_isse(BYTE * motion, int height, int width_mod8, int pitch,   int * mcount )
{
	...
	__asm
	{
		...
		pxor	mm3, mm3 // zero general counter
		pxor	mm0, mm0 // mm0 = constant 0
		xor		edi, edi // height variable
Hcycle:
		xor		eax, eax // width variable
ALIGN 16
Wcycle:
		movq	mm1,[esi + eax]     // all eight bytes from p  into mm0
		pcmpeqb	mm1, mm0 // =1111111 (i.e. -1) for motion=0,   =0 for motion not 0.
		pcmpeqb mm1, mm0 // =0 for motion=0,   =1111111 for motion not 0
		pxor	mm2, mm2 // mm2=0
		psubb	mm2, mm1 // = 0 for motion=0,  = 1 for motion not 0
		psadbw	mm2, mm0 // sum, i.e. count
		paddd	mm3, mm2 // add to general counter

		add		eax, 8
		cmp		eax, ecx // < width_mod8?
		jl		Wcycle

		add		esi, edx // +pitch  - bug fixed in v.3.3.1
		add		edi, 1
		cmp		edi, ebx // < heigth?
		jl		Hcycle
...
//	motion_count(motion, parms.height, width_mod8, parms.pitch, &count);
	motion_count_isse(motion, parms.height, width_mod8, parms.pitch, &count);
#else
	motion_count(motion, parms.height, width_mod8, parms.pitch, &count);
#endif
...
{	// add external mask of good objects (for example SAD<threshold) to motion - added in v.3.5
	...
	__asm
	{
		...
		pcmpeqb	mm7, mm7 // useful constant FFFFFFF (-1)
		pxor	mm0, mm0 // 00000000
		xor		ebx, ebx // height variable=0

Hcycle:
		xor		eax, eax // width variable=0
ALIGN 16
Wcycle:
		movq	mm1, [esi + eax]     // 8 bytes from ext
		pcmpgtb	mm1, mm7 // signed compare =FFFFFFFFF for <=127 (for positive 0 to 127),   =0 for >127 (for negative -127 to -1).
		pandn	mm1, mm7 // NOT
		movq	mm2, [edi + eax]     // 8 bytes from motion
		por		mm2, mm1 // ext>127 OR motion
		movq	[edi + eax], mm2 // write result to motion
		add		eax, 8
		cmp		eax, ecx // < width?
		jl		Wcycle

		mov		edx, [ext_pitch]
		add		esi, edx // +ext_pitch
		mov		edx,[pitch]
		add		edi, edx // +pitch
		mov		edx, [height]
		add		ebx, 1
